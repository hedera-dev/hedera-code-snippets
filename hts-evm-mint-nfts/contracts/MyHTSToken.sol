...
contract MyHTSToken is HederaTokenService, KeyHelper, Ownable {
    ...
    // --------------------------------------------------------------------------------
    // Hedera-specific association helper
    // --------------------------------------------------------------------------------

    // EOAs can self-associate before receiving NFTs if they don’t have auto-association
    function associateToCaller() external {
        int rc = associateToken(msg.sender, tokenAddress);
        require(
